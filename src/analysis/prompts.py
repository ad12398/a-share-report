...
## 输出格式（必须严格遵守，不得合并为一个段落）

请用 <h3> 分节输出以下两节，每节至少 2 句话：

<h3>一、隔夜传导</h3>
<p>分析隔夜美股（纳斯达克）表现对 A 股开盘的传导效应，结合富时 A50 期货的指向意义判断开盘方向。若隔夜数据缺失则注明。</p>

<h3>二、今日关注</h3>
<p>结合昨日收盘指数位置（参考上方对比数据）、当前宏观背景（CPI/PPI/PMI/M2/LPR 等）、数据发布日历，给出今日需要关注的风险点、板块和关键位置。不要写成流水账，要有量化视角。</p>
